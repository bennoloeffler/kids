...
(defn render-laser [ship]
  (let [laser (:laser ship)
        reallaser (+ 20 (- laser))
        shot (* 10 reallaser)
        laserlength 30
        shotx shot
        shotxx (+ laserlength shot)]

   (when (> laser 0)
     (q/stroke 250)
     (q/line shotx 0 shotxx 5))))
...
(defn update-state [state]
  (-> state
      ...
      emit-smoke
      (update-in [:ship :laser] age-laser)
...
(defn on-key-down [state event]
  (case (:key event)
    ...
    (:l :k)     (assoc-in state [:ship :laser] 20)
    state))

(defn on-key-up [state _]
